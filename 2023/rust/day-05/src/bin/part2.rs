...
fn process(input: &str) -> u64 {
    let almanac = Almanac::new_from_aoc_input_v2(input);

    println!("Flattening seed intervals");
    let seeds = almanac
        .seeds_v2
        .iter()
    ...
    let locations = seeds.into_par_iter().progress().map(|seed| {
