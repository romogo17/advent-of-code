...
fn parse_records(input: &str) -> IResult<&str, Vec<(String, Vec<u64>)>> {
    let (input, output) = all_consuming(many1(terminated(
        separated_pair(
            is_a("#.?").map(|line| std::iter::repeat(line).take(5).join("?")),
            space1,
            separated_list1(tag(","), complete::u64)
                .map(|groups| std::iter::repeat(groups).take(5).flatten().collect()),
        ),
        multispace0,
    )))(input)?;
...
?###???????? 3,2,1";
    ...
    fn input1() -> miette::Result<()> {
        ...
        assert_eq!(6512849198636, process(input)?);
        Ok(())
    }
}
