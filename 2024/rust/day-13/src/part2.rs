...
fn a_button(input: &str) -> IResult<&str, I64Vec2> {
    preceded(
        tag("Button A: X+"),
        separated_pair(complete::i64, tag(", Y+"), complete::i64).map(|(x, y)| I64Vec2::new(x, y)),
    )(input)
}

fn b_button(input: &str) -> IResult<&str, I64Vec2> {
    preceded(
        tag("Button B: X+"),
        separated_pair(complete::i64, tag(", Y+"), complete::i64).map(|(x, y)| I64Vec2::new(x, y)),
    )(input)
...
fn prize(input: &str) -> IResult<&str, I64Vec2> {
    preceded(
        tag("Prize: X="),
...
fn solve(claw_machine: &ClawMachine) -> (i64, i64) {
...
Prize: X=18641, Y=10279";
    ...
    fn input() -> miette::Result<()> {
        ...
        assert_eq!(92572057880885, process(input)?);
        Ok(())
    }
}
